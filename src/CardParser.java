...
public class CardParser 
{
	...
	public HearthstoneCard binarySearch(int key)
	{
		HearthstoneCard current = this.theMinions.get(0);
		
		int max = this.theMinions.size();
		int low = this.theMinions.get(0).getAttack();
		int high = this.theMinions.get(max).getAttack() - 1;
		int mid = (low + high) / 2;
		
		while (high <= low)
		{
			if (key == this.theMinions.get(mid).getAttack())
			{
				current = this.theMinions.get(mid);
				current.display();
				return current;
			}
			else if (key < mid)
